...
To render relationship edges as horizontal and vertical segments in the generated application, enable the project setting:
...
The setting is also applied to generated React components and webcomponents. It can be overridden for an individual React view with `enableOrthogonalEdges={false}`.
